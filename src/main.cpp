...
#if defined(NDEBUG)
# error "Citicoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x21599de3901df33786822c4c6c0452e22763fe3a525947b054a4c97cd4569288");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Citicoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Citicoin Signed Message:\n";
...
{
    // Citicoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 500000); // Citicoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Citicoin: 1 days
static const int64 nTargetSpacing = 2.5 * 60; // Citicoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Citicoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Citicoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x678ed68537036a5741cd414a97f9e466a165e6ed0b5da81132a0c1a2fc4bddec");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Citioption is trading technologies";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04ed7afab2b5d4bd48b50cb5134b08b7fa8b8485fdf85ec7bbade5249ce545fb7225b946ed4602822efd9554b64db91484be74501a90d37801819e49c3a44c7c8d") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1527749504;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084732157;

        if (fTestNet)
        {
            block.nTime    = 1527749476;
            block.nNonce   = 386104660;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xe5aaad755727414bc1b4ea241b5abbbf241bf20f2f4d33077fbecac0c162a103"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf7, 0xc3, 0xb9, 0xd6 }; // Citicoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("CiticoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("CiticoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static CiticoinMiner(CWallet *pwallet)
{
    printf("CiticoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("citicoin-miner");
    ...
    try { loop {
        ...
        printf("Running CiticoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("CiticoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&CiticoinMiner, pwallet));
}
